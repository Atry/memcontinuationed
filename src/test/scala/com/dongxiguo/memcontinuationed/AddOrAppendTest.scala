...
object AddOrAppendTest {
  implicit val (logger, formatter, appender) = ZeroLoggerFactory.newLogger(this)
}

class AddOrAppendTest {
  import AddOrAppendTest._
  ...
  def stored() {
    synchronized {
      ...
      val accessor = new UTF8Accessor("memcontinuationed_addOrAppend_test_stored")
      ...
      reset {
        memcontinuationed.addOrAppend(accessor, "1")
        if (memcontinuationed.require(accessor) == "1") {
          memcontinuationed.addOrAppend(accessor, "2")
