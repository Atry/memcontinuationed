...
class PrependTest {
  ...
  def stored() {
    synchronized {
      ...
      val accessor = new UTF8Accessor("memcontinuationed_prepend_test_stored")
      ...
      reset {
        ...
        memcontinuationed.prepend(accessor, "0")
  ...
  def notStored() {
    synchronized {
      ...
      val channelGroup = AsynchronousChannelGroup.withThreadPool(executor)
      val memcontinuationed = new Memcontinuationed(channelGroup, TestServerAddress.getAddress _)
      val accessor = new UTF8Accessor("memcontinuationed_prepend_test_not_stored")
