...
class ReplaceTest {
  ...
  def stored() {
    synchronized {
      ...
      val accessor = new UTF8Accessor("memcontinuationed_replace_test_stored")
      ...
      reset {
        ...
        memcontinuationed.replace(accessor, "2")
  ...
  def notStored() {
    synchronized {
      ...
      val channelGroup = AsynchronousChannelGroup.withThreadPool(executor)
      val memcontinuationed = new Memcontinuationed(channelGroup, TestServerAddress.getAddress _)
      val accessor = new UTF8Accessor("memcontinuationed_replace_test_not_stored")
