...
class AddTest {
  ...
  def stored() {
    synchronized {
      ...
      val accessor = new UTF8Accessor("memcontinuationed_add_test_stored")
  ...
  def notStored() {
    synchronized {
      ...
      val channelGroup = AsynchronousChannelGroup.withThreadPool(executor)
      val memcontinuationed = new Memcontinuationed(channelGroup, TestServerAddress.getAddress _)
      val accessor = new UTF8Accessor("memcontinuationed_add_test_not_stored")
      ...
      reset {
        memcontinuationed.add(accessor, "1")
        memcontinuationed.add(accessor, "2") {
